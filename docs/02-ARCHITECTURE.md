...
|                                                                                   |
|   Target Entrypoint: `coworkers` (cmd/coworkers - IMPLEMENTED: doctor/attach/status; run UNSUPPORTED) |
...
- **Phân tách quá độ**: `cmd/recovery` hiện là công cụ phục hồi hẹp và được duy trì tính tương thích ngược; mã nguồn entrypoint đặt tại `cmd/coworkers` (IMPLEMENTED cho doctor/attach/status; `coworkers run` vẫn UNSUPPORTED, exit 1) mà không sao chép logic recovery.
